...
public class MB_MigrateMaterialsToDifferentPipeline : MonoBehaviour
{
    ...
    string GetRelativeExamplesPath(){
        ...
        string exampleScene = "BatchPrepareObjectsForDynamicBatching";
        string[] targetGID = AssetDatabase.FindAssets(exampleScene + " t:SceneAsset");
        ...
        relativeAssetPath = relativeAssetPath.Substring(0, relativeAssetPath.Length - ("/".Length + exampleScene.Length + ".unity".Length));
        relativeAssetPath = relativeAssetPath.Replace('\\', '/');
        relativeAssetPath = relativeAssetPath.Replace("//", "/");
        string examplesPath = relativeAssetPath;
    ...
    int GetMaterialPipelineType(){
        ...
        string pathToTestMat = relativePath + "/Assets/Materials/firstFloorPlatform.mat";
        Material targetMat = AssetDatabase.LoadAssetAtPath<Material>(pathToTestMat);
        if(targetMat == null){
